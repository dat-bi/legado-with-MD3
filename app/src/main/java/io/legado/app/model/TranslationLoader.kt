...
object TranslationLoader {
        ...
        try {
            ...
            try {
                ...
                val vietPhraseTrie = try {
                    loadDictionary(
                        binaryAsset = "dict/vietphrase.bin",
    ...
    private suspend fun loadPhoneticDictionary(
        ...
        if (DictManager.hasCustomDict(customType)) {
            ...
            if (customBin.exists()) {
                try {
                    ...
                    return@withContext FileInputStream(customBin).use { loadPhoneticBinary(it) }
                } catch (e: Exception) {
        ...
        if (defaultBin.exists()) {
            try {
                ...
                return@withContext FileInputStream(defaultBin).use { loadPhoneticBinary(it) }
            } catch (e: Exception) {
        ...
        // Try pre-built phonetic asset
        try {
            android.util.Log.d("TranslationLoader", "Trying to load pre-built phonetic asset: dict/phienam.bin")
            appCtx.assets.open("dict/phienam.bin").use { stream ->
                val map = loadPhoneticBinary(stream)
                try {
                    defaultBinDir.mkdirs()
                    savePhoneticBinary(map, defaultBin)
                } catch (e: Exception) {
                    android.util.Log.w("TranslationLoader", "Failed to cache pre-built phonetic", e)
                }
                return@withContext map
            }
        } catch (e: Exception) {
            android.util.Log.w("TranslationLoader", "Failed to load pre-built phonetic asset", e)
        }
    ...
    private fun loadPhoneticBinary(inputStream: InputStream): Map<String, String> {
